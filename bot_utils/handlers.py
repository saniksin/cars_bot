...
async def get_cars_by_name(message: types.Message):
    text = """
    Пожалуйста отправьте название модели или марки машины, которую хотите найти.
    """
...
async def delete_old_posts(message: types.Message):
    ...
    await message.answer("Начинаю удалять старые посты...")
    manager.delete_post(old_time)
    await message.answer("Посты созданные больше 30 дней будут удалены.")